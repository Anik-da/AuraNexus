...
export default function AuraShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-transparent text-[#f4f4f5] flex font-sans antialiased overflow-hidden relative">
      ...
      <aside className="hidden lg:flex flex-col w-64 border-r border-zinc-800 bg-[#09090b]/95 backdrop-blur-xl shrink-0 h-screen sticky top-0 relative overflow-hidden z-20">
        ...
        {/* Brand Header */}
        <div className="h-16 px-6 border-b border-zinc-800 flex items-center gap-3.5 shrink-0 relative z-10">
          <div className="w-5 h-5 rounded-full bg-[#ff3344]/10 border border-[#ff3344]/40 flex items-center justify-center shadow-[0_0_8px_rgba(255,51,68,0.25)]">
          ...
          {SIDEBAR_LINKS.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} className="block relative">
                <button className={`w-full flex items-center gap-3.5 px-4 py-2.5 rounded-lg font-sans text-xs sm:text-sm font-medium transition-all duration-200 border ${
                  active
                    ? "border-zinc-800 bg-zinc-900/80 text-white font-semibold shadow-[0_4px_20px_rgba(0,0,0,0.4)]"
                    : "border-transparent text-zinc-400 hover:text-white hover:bg-zinc-900/30"
                }`}>
                  <Icon className={`w-[18px] h-[18px] transition-colors ${active ? "text-[#ff3344] drop-shadow-[0_0_4px_rgba(255,51,68,0.4)]" : "text-zinc-500"}`} />
                  <span>{link.label}</span>
                </button>
                {active && (
                  <div className="absolute left-0 top-2.5 bottom-2.5 w-0.5 bg-[#ff3344] rounded-r-md shadow-[0_0_8px_rgba(255,51,68,0.8)]"></div>
                )}
              </Link>
            );
          })}
        ...
        {/* Footer Status info */}
        <div className="p-4 border-t border-zinc-800 font-mono text-[8px] text-[#ff3344]/60 uppercase tracking-widest shrink-0 relative z-10 flex items-center gap-2 pl-6">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden bg-transparent relative z-10">
        ...
        {/* Top Status Bar */}
        <header className="h-16 border-b border-zinc-800 bg-[#09090b]/40 backdrop-blur-md flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <span className="text-[9px] font-mono text-zinc-550 uppercase tracking-wider">SYS_MODE:</span>
          ...
          <div className="flex items-center gap-5 md:gap-6">
            ...
            <div className="flex items-center gap-2 border-l border-zinc-800 pl-5 md:pl-6">
              ...
              <span className="text-[9px] font-mono text-zinc-350 uppercase tracking-widest hidden sm:inline">
                Commander Lex
              </span>
            </div>
          </div>
        ...
        <section className="h-10 border-b border-zinc-800 bg-gradient-to-r from-[#28090b]/20 to-transparent flex items-center px-6 shrink-0 relative overflow-hidden select-none">
          ...
          <div className="flex-1 overflow-hidden relative" style={{ maskImage: "linear-gradient(90deg, transparent, black 10%, black 90%, transparent)" }}>
            <div className="ticker-track-animation flex whitespace-nowrap gap-12 text-[10px] font-mono text-zinc-500">
          ...
          {mobileMenuOpen && (
            <>
              ...
              <motion.aside
                ...
                className="lg:hidden fixed top-0 bottom-0 left-0 w-64 bg-[#09090b] border-r border-zinc-800 z-50 flex flex-col"
              >
                ...
                <nav className="flex-1 px-4 py-6 space-y-2">
                  {SIDEBAR_LINKS.map((link) => {
                    ...
                    const active = pathname === link.href;
                    return (
                      <Link key={link.href} href={link.href} className="block relative" onClick={() => setMobileMenuOpen(false)}>
                        <button className={`w-full flex items-center gap-3.5 px-4 py-2.5 rounded-lg font-sans text-xs sm:text-sm font-medium transition-all border ${
                          active
                            ? "border-zinc-800 bg-zinc-900/80 text-white font-semibold shadow-[0_4px_20px_rgba(0,0,0,0.4)]"
                            : "border-transparent text-zinc-400 hover:text-white hover:bg-zinc-900/30"
                        }`}>
                          <Icon className={`w-[18px] h-[18px] ${active ? "text-[#ff3344]" : "text-zinc-500"}`} />
                          <span>{link.label}</span>
                        </button>
                        {active && (
                          <div className="absolute left-0 top-2.5 bottom-2.5 w-0.5 bg-[#ff3344] rounded-r-md shadow-[0_0_8px_rgba(255,51,68,0.8)]"></div>
